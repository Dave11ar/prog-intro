...
public class WordStatSortedLastIndex {
    public static void main(String[] args) throws IOException {
        ...
        Map<String, Integer> siz = new LinkedHashMap<>();
        ...
        while (sc.hasNextLine()) {
            ...
            while (line.hasNextWord()) {
                ...
                siz.put(str, siz.get(str) + 1);
                last.put(str, number);
                number++;
            }
            for (Map.Entry<String, Integer> entry : hashmap.entrySet()) {
                if (last.get(entry.getKey()) != null) {
                    vec.get(entry.getValue()).push_back(last.get(entry.getKey()));
                }
                
                last.put(entry.getKey(), null);
            }
            str_num++;
        }
        ...
        try(BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(args[1]), "utf8"));){
            for (int i = 0; i < list.size(); i++) {
                ...
                bufferedWriter.write(siz.get(list.get(i).key) + "");
                
                for (int j = 0; j < vec.get(list.get(i).line).last; j++) {
                    bufferedWriter.write(" ");
                    bufferedWriter.write(vec.get(list.get(i).line).get(j) + "");
                }
